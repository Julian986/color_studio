...
function buildServiceItemsForInput(input: CreateReservationInput): {
  ...
  if (!isValidServiceSelection(comboIds)) return null;
  if (comboIds.length > 0) {
    const treatments = comboIds
      .map((id) => findSalonTreatmentById(id))
      .filter((t): t is NonNullable<typeof t> => Boolean(t));
    if (treatments.length !== comboIds.length) return null;
...
export async function insertPanelReservation(
  ...
  const serviceIds = normalizeServiceIds(
    input.serviceIds?.length ? input.serviceIds : [input.treatmentId.trim()],
  );
  if (!isValidServiceSelection(serviceIds)) {
    return { error: "Tratamiento inválido.", code: "INVALID_TREATMENT" };
  }

  const treatments = serviceIds
    .map((id) => findSalonTreatmentById(id))
    .filter((t): t is NonNullable<typeof t> => Boolean(t));
  if (treatments.length !== serviceIds.length) {
    return { error: "Tratamiento inválido.", code: "INVALID_TREATMENT" };
  }

  const primaryTreatment = treatments[0] as SalonTreatment;
  const isCombo = treatments.length > 1;
  const serviceItems = treatments.map((t) => ({
    treatmentId: t.id,
    treatmentName: t.name,
    subtitle: t.subtitle,
    category: t.category,
